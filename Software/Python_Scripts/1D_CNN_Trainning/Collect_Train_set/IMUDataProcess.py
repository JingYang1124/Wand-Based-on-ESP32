...
def UpdatePositionAndSave(SpellName,raw_ax,raw_ay,raw_az,raw_gx,raw_gy,raw_gz):
    global after_ax
    global after_ay
    global after_az
    global cut_index_1
    ...
    plt.close()
    while(1):
        command = input('Save the data to Train? Y or N or R(Yes or No or Re-display) :')
        if command is "Y":
            print("Now save the data to Train")
            WriteDataToTrainSet(SpellName,final_x,final_y,final_z)
            break
        if command is "N":
            break
        if command is "R":
            ...
            ax.set_ylim((axis_min_y,axis_max_y))
            ax.set_zlim((axis_min_z,axis_max_z))
            plt.pause(6)  # 暂停一段时间，不然画的太快会卡住显示不出来
            plt.close()
